    ...
    return RPCHelpMan{
        ...
        RPCResult{
            RPCResult::Type::OBJ, "", "",
            {
                ...
                {RPCResult::Type::BOOL, "ispostquantum", /*optional=*/true, "If the address is a post-quantum Dilithium (ML-DSA-65) address, i.e. spendable on QubitCoin"},
                {RPCResult::Type::NUM, "witness_version", /*optional=*/true, "The version number of the witness program"},
                {RPCResult::Type::STR_HEX, "witness_program", /*optional=*/true, "The hex value of the witness program"},
                {RPCResult::Type::ARR, "warnings", /*optional=*/true, "Warnings about the address; present when it is well-formed but unspendable on this chain",
                    {
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            if (isValid) {
                ...
                // A well-formed address for this network can still be a dead
                // end: only Dilithium destinations are spendable, so say so
                // here rather than letting "isvalid: true" imply usable.
                if (!IsDilithiumDestination(dest)) {
                    UniValue warnings(UniValue::VARR);
                    warnings.push_back(strprintf(
                        "Address type is not supported on QubitCoin (Dilithium-only chain). "
                        "Sending to this address would burn the funds: it is a %s address, and no "
                        "key that can exist on this chain is able to spend it.",
                        DestinationTypeName(dest)));
                    ret.pushKV("warnings", std::move(warnings));
                }
            } else {
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            for (int64_t i = range_begin; i <= range_end; ++i) {
                ...
                for (const CScript& script : scripts) {
                    ...
                    // pkh() / addr(<legacy P2PKH>) must keep the leftover ECDSA
                    // version byte. ExtractDestination alone would upgrade them
                    // to Dilithium and weaken burn protection.
                    dest = PreserveDescriptorAddressType(std::move(dest), desc->GetOutputType());
