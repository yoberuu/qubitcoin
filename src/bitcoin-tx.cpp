...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " qbitcoin-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "Usage:  qbitcoin-tx [options] <hex-tx> [commands]  Update hex-encoded QubitCoin transaction\n"
                "or:     qbitcoin-tx [options] -create [commands]   Create hex-encoded QubitCoin transaction\n"
                "\n";
