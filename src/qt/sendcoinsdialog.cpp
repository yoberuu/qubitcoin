...
void SendCoinsDialog::setModel(WalletModel *_model)
{
    ...
    if(_model && _model->getOptionsModel())
    {
        ...
        if (model->wallet().hasExternalSigner()) {
            ...
            ui->sendButton->setToolTip(tr("Creates a Partially Signed QubitCoin Transaction (PSBT) for use with e.g. an offline %1 wallet, or a PSBT-compatible hardware wallet.").arg(PACKAGE_NAME));
        }
...
bool SendCoinsDialog::PrepareSendText(QString& question_string, QString& informative_text, QString& detailed_text)
{
    ...
    if (model->wallet().privateKeysDisabled() && !model->wallet().hasExternalSigner()) {
        ...
        question_string.append(tr("Please, review your transaction proposal. This will produce a Partially Signed QubitCoin Transaction (PSBT) which you can save or copy and then sign with e.g. an offline %1 wallet, or a PSBT-compatible hardware wallet.").arg(PACKAGE_NAME));
        ...
        question_string.append(tr("Please, review your transaction. You can create and send this transaction or create a Partially Signed QubitCoin Transaction (PSBT), which you can save or copy and then sign with, e.g., an offline %1 wallet, or a PSBT-compatible hardware wallet.").arg(PACKAGE_NAME));
    } else {
...
void SendCoinsDialog::processSendCoinsReturn(const WalletModel::SendCoinsReturn &sendCoinsReturn, const QString &msgArg)
{
    ...
    case WalletModel::InvalidAddress:
        ...
        break;
    case WalletModel::UnspendableAddress:
        msgParams.first = tr("Address type is not supported on QubitCoin (Dilithium-only chain). Sending to this address would burn the funds. Ask the recipient for a post-quantum Dilithium address.");
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        else if (!IsDilithiumDestination(dest)) // Well-formed, but unspendable here
        {
            // Leave destChange unset so the wallet falls back to a Dilithium
            // change address instead of burning the change.
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Not a Dilithium address, the change would be burned"));
        }
        else // Valid address
        {
