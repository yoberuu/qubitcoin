...
const char * const BITCOIN_CONF_FILENAME = "qubitcoin.conf";
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\QubitCoin
    //   new: C:\Users\Username\AppData\Local\QubitCoin
    // macOS: ~/Library/Application Support/QubitCoin
    // Unix-like: ~/.qubitcoin
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "QubitCoin";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "QubitCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/QubitCoin";
#else
    // Unix-like
