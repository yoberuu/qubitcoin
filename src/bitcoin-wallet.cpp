...
static void SetupWalletToolArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-descriptors", "Create a descriptor wallet (ECDSA-only; not for normal QubitCoin use). Only for 'create'", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    argsman.AddArg("-legacy", "Create a legacy Dilithium-capable wallet (the default when neither flag is given). Only for 'create'", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
...
static std::optional<int> WalletAppInit(ArgsManager& args, int argc, char* argv[])
{
    ...
    if (missing_args || HelpRequested(args) || args.IsArgSet("-version")) {
        std::string strUsage = strprintf("%s qbitcoin-wallet version", PACKAGE_NAME) + " " + FormatFullVersion() + "\n";

        if (args.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                        "qbitcoin-wallet is an offline tool for creating and interacting with " PACKAGE_NAME " wallet files.\n"
                        "By default qbitcoin-wallet will act on wallets in the default mainnet wallet directory in the datadir.\n"
                        "To change the target wallet, use the -datadir, -wallet and -regtest/-signet/-testnet/-testnet4 arguments.\n\n"
                        "Usage:\n"
                        "  qbitcoin-wallet [options] <command>\n";
            strUsage += "\n" + args.GetHelpMessage();
        }
